...
public class Evaluation<T extends Comparable<? super T>> implements Serializable {
    ...
    /*
     * Produced by Kong Sungeon
     */
    public double QWKappa(){
    	double grade[][] = new double[numOfClasses][numOfClasses];
    	String actual, expected;
        List<Integer> classes = confusion.getClasses();
        for (Integer clazz : classes) {
            actual = resolveLabelForClass(clazz);
        ...
        CalKappa ck = new CalKappa();
    	double kappa = ck.CalKappa(grade, numOfClasses);
    	return kappa;
    }
    
    
    
    public String resolveLabelForClass(Integer clazz) {
